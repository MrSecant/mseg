...
import mseg_api.mseg.utils.names_utils as names_utils
import mseg_api.mseg.utils.resize_util as resize_util
from mseg_api.mseg.utils.dir_utils import check_mkdir, create_leading_fpath_dirs
from mseg_api.mseg.utils.mask_utils_detectron2 import Visualizer
from mseg_api.mseg.taxonomy.taxonomy_converter import TaxonomyConverter
from mseg_api.mseg.taxonomy.naive_taxonomy_converter import NaiveTaxonomyConverter
...
class InferenceTask:
    ...
    def execute(self) -> None:
        ...
        if is_img:
            ...
            test_loader, self.data_list = create_test_loader(self.args)
            self.execute_on_dataloader(test_loader)
            write_file_list(self.args.file_list_txt, self.data_list, self.mseg_list)
        elif is_vid:
            # argument is a video
    ...
    def execute_on_dataloader(self, test_loader: torch.utils.data.dataloader.DataLoader) -> None:
        ...
        if self.args.save_folder == "default":
            self.args.save_folder = f"{_ROOT}/nuscene/{self.args.model_name}_{self.args.dataset}_universal_{self.scales_str}/{self.args.base_size}"
        ...
        for i, (input, _) in enumerate(test_loader):
            ...
            g_f = os.path.join(self.gray_folder, self.args.data_root.split('/')[-3])
            os.makedirs(g_f, exist_ok=True)

            gray_path = os.path.join(self.gray_folder, self.args.data_root.split('/')[-3], image_name + ".png")
            # if Path(gray_path).exists():
            #     continue
            self.mseg_list.append(gray_path)
...
def write_file_list(file_list, data_list, mseg_list):
    f = open(file_list, 'a')
    for i in range(len(data_list)):
        f.write(data_list[i][0])
        f.write(',')
        f.write(mseg_list[i])
        f.write(',')
        f.write(data_list[i][0])
